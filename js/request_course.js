...
function handleAddSection(event) {
    ...
    .then(data => {
        if (data.status === 'success') {
            addSelectedCourseToUI(courseId, sectionNo);
        } else {
            alert(data.message);
        }
    })
...
function addSelectedCourseToUI(courseId, sectionNo) {
    const sectionItems = document.querySelectorAll('.section-item');
    let sectionData = null;
    
    for (const item of sectionItems) {
        const itemCourseId = item.querySelector('.add-btn').getAttribute('data-section-code');
        const itemSectionNo = item.querySelector('.add-btn').getAttribute('data-section-no');
        
        if (itemCourseId === courseId && itemSectionNo === sectionNo) {
            const sectionCode = item.querySelector('.section-code').textContent;
            const sectionTime = item.querySelector('.section-time').textContent;
            const sectionSeats = item.querySelector('.section-seats').textContent;
    ...
    const sectionCode = document.createElement('span');
    sectionCode.className = 'section-code';
    sectionCode.textContent = sectionData.sectionCode;
    
    const sectionTime = document.createElement('span');
    sectionTime.className = 'section-time';
    sectionTime.textContent = sectionData.sectionTime;
    
    const sectionSeats = document.createElement('span');
    sectionSeats.className = 'section-seats';
    sectionSeats.textContent = sectionData.sectionSeats;
    ...
    removeButton.className = 'remove-btn';
    removeButton.textContent = 'Remove';
    removeButton.setAttribute('data-section-code', courseId);
    removeButton.setAttribute('data-section-no', sectionNo);
