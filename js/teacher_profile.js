...
async function fetchTeacherData() {
    try {
        const response = await fetch('../php/Profile/teacher_profile.php');
        if (!response.ok) {
        ...
        alert('Failed to load teacher data. Please refresh the page or contact support.');
        return null;
    }
}

function populateTeacherProfile(data) {
    if (!data || !data.teacher) {
        console.error('No teacher data available to populate profile');
        return;
    }
    ...
    document.getElementById('firstName').value = data.teacher.FirstName || 'N/A';
    document.getElementById('middleName').value = data.teacher.MiddleName || 'N/A';
    document.getElementById('lastName').value = data.teacher.LastName || 'N/A';
    document.getElementById('email').value = data.teacher.Email || 'N/A';
    document.getElementById('phone').value = data.teacher.PhoneNumber || 'N/A';
    
    // Address Information
    document.getElementById('houseNumber').value = data.teacher.HouseNumber || 'N/A';
    document.getElementById('streetNumber').value = data.teacher.StreetNumber || 'N/A';
    document.getElementById('streetName').value = data.teacher.StreetName || 'N/A';
    document.getElementById('postalCode').value = data.teacher.PostalCode || 'N/A';

    //City
    if (data.teacher.PostalCode) {
        const postalCode = parseInt(data.teacher?.PostalCode);
    ...
    document.getElementById('departmentDisplay').textContent = data.teacher.Department || 'N/A';
    
    // Profile Summary
    document.getElementById('teacherName').textContent = 
        `${data.teacher.FirstName || ''} ${data.teacher.MiddleName || ''} ${data.teacher.LastName || ''}`.trim() || 'N/A';
    document.getElementById('teacherId').textContent = `ID: ${data.teacher.TeacherID || 'N/A'}`;
    document.getElementById('teacherDept').textContent = `Department: ${data.teacher.Department || 'N/A'}`;

    // Teaching Summary
    document.getElementById('sidebarDepartment').textContent = `${data.teacher.Department || 'N/A'}`;
}
...
function enableEditing() {
    ...
    // Dept Dropdown
    if (!profileData.teacher.Department && profileData.departments) {
        const dropdownHTML = `
...
async function saveChanges() {
    try {
        ...
        if (!profileData.teacher.Department) {
        ...
        if (result.success) {
            const newData = await fetchTeacherData();
            populateTeacherProfile(newData);
            toggleEditMode();
...
function cancelChanges() {
    populateTeacherProfile(profileData);
    disableEditing();
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (teacherData) {
            populateTeacherProfile(teacherData);
